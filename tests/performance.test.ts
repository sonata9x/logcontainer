...
const takoyakiPlatformMigration = readFileSync(new URL("../supabase/migrations/202608310001_takoyaki_box_platform.sql", import.meta.url), "utf8");
...
test("public routes bypass auth refresh and stored documents use lightweight reads", () => {
  ...
  const takoyakiPlatformMarker = "-- 202608310001_takoyaki_box_platform.sql";
  ...
  const sidebarReorderMarker = "-- 202609010001_sidebar_reordering.sql";
  ...
  assert.equal(normalizedSql(schema.slice(schema.indexOf(finalPolishMarker) + finalPolishMarker.length, schema.indexOf(takoyakiPlatformMarker))), normalizedSql(finalPolishMigration));
  ...
  assert.equal(normalizedSql(schema.slice(schema.indexOf(takoyakiAvatarMarker) + takoyakiAvatarMarker.length, schema.indexOf(sidebarReorderMarker))), normalizedSql(takoyakiAvatarMigration));
  assert.equal(normalizedSql(schema.slice(schema.indexOf(sidebarReorderMarker) + sidebarReorderMarker.length)), normalizedSql(sidebarReorderMigration));
});
