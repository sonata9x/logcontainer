...
import { BookOpen, CopyPlus, ImagePlus, Pencil, Plus, Trash2, X } from "lucide-react";
...
import type { Handout, LogFontFamily } from "@/lib/types";
import { useEscapeClose } from "@/lib/use-escape-close";

type Props = ({ mode: "editor"; pageId: string; canEdit: boolean } | { mode: "public" | "guest"; token: string }) & { fontFamily?: LogFontFamily };
type PendingImage = { file: File; preview: string };
type HandoutSource = { pageId: string; pageTitle: string; handouts: Array<{ id: string; title: string }> };

function apiBase(props: Props) {
  return props.mode === "editor" ? `/api/pages/${props.pageId}/handouts` : props.mode === "public" ? `/api/publications/${encodeURIComponent(props.token)}/handouts` : `/api/share/${encodeURIComponent(props.token)}/handouts`;
}
...
export function HandoutLibrary(props: Props) {
  ...
  const [sources, setSources] = useState<HandoutSource[]>([]);
  ...
  async function openCopy() {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/pages/${props.pageId}/handouts/copy`, { cache: "no-store" });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result.error ?? "가져올 핸드아웃을 불러오지 못했습니다.");
      setSources(result.sources ?? []);
    } catch (error) {
      setCopying(false);
      window.alert(error instanceof Error ? error.message : "가져올 핸드아웃을 불러오지 못했습니다.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function copySelected() {
    if (!editor || !selectedSourceIds.length) return;
    setSaving(true);
    try {
      const response = await fetch(`/api/pages/${props.pageId}/handouts/copy`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ handoutIds: selectedSourceIds }) });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result.error ?? "핸드아웃을 가져오지 못했습니다.");
      setCopying(false);
      await load();
    } catch (error) {
      window.alert(error instanceof Error ? error.message : "핸드아웃을 가져오지 못했습니다.");
    } finally {
      setSaving(false);
    }
  }
