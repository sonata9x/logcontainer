...
export function BgmSourceCreator({ onCreated }: { onCreated: () => void | Promise<void> }) {
  ...
  return <div className="bgm-source-creator"><button className="button" onClick={() => input.current?.click()} disabled={pending}><Upload size={14} />새 MP3</button><input ref={input} className="visually-hidden" type="file" accept="audio/mpeg,.mp3" onChange={(event) => { const file = event.target.files?.[0]; event.target.value = ""; if (file) void uploadFile(file); }} /><form className="bgm-youtube-form" onSubmit={addYoutube}><input name="title" placeholder="YouTube BGM 제목" maxLength={200} required /><input name="youtubeUrl" placeholder="YouTube 주소" required /><button className="button" disabled={pending}><Plus size={14} />추가</button></form></div>;
}
