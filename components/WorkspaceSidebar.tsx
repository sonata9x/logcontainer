...
import { previewSiblingResourceReorder, wouldCreateResourceCycle, type ResourceReorder } from "@/lib/resources/reorder";
...
export function WorkspaceSidebar({ workspaceId, workspaceName, nickname, pages, isSiteAdmin }: { workspaceId: string; workspaceName: string; nickname: string; accentColor: string; pages: WorkspacePage[]; isSiteAdmin: boolean }) {
  const { systemFont, setSystemFont, accentColor: currentAccentColor, setAccentColor } = useWorkspaceAppearance();
  ...
  return <>
    ...
    <div className="sidebar-footer"><TrashPanel onChanged={reloadTree} /><button className="sidebar-action" onClick={() => setSettingsOpen(true)}><Settings size={15} />설정</button>{isSiteAdmin && <Link className="sidebar-action" href="/workspace/admin/accounts"><ShieldCheck size={15} />관리</Link>}<button className="sidebar-action" onClick={logout}><LogOut size={15} />로그아웃</button></div>
    {settingsOpen && <WorkspaceSettingsDialog workspaceName={currentWorkspaceName} nickname={currentNickname} accentColor={currentAccentColor} systemFont={systemFont} onClose={() => setSettingsOpen(false)} onSaved={(next) => { setCurrentWorkspaceName(next.workspaceName); setCurrentNickname(next.nickname); setSystemFont(next.systemFont); setAccentColor(next.accentColor); setSettingsOpen(false); }} />}
...
function WorkspaceSettingsDialog({ workspaceName, nickname, accentColor, systemFont, onClose, onSaved }: { workspaceName: string; nickname: string; accentColor: string; systemFont: LogFontFamily; onClose: () => void; onSaved: (next: { workspaceName: string; nickname: string; accentColor: string; systemFont: LogFontFamily }) => void }) {
  const [settingsTab, setSettingsTab] = useState<"general" | "account" | "bgm">("general");
...
function PageNode({ page, pages, depth, createPage, reloadTree }: { page: WorkspacePage; pages: WorkspacePage[]; depth: number; createPage: CreatePage; reloadTree: () => Promise<void> }) {
  ...
  const invalidFolderTarget = page.page_type === "folder" && wouldCreateResourceCycle(page.id, treeInteraction?.draggingIds ?? [], pages);
  ...
  const row = <div className={`page-link tree-row ${href && pathname === href ? "active" : ""} ${selected ? "selected" : ""} ${dragging ? "dragging" : ""} ${dropTarget ? "drop-target" : ""} ${dropBefore ? "drop-before" : ""} ${dropAfter ? "drop-after" : ""}`} style={{ paddingLeft }} data-resource-id={page.id} onClick={(event) => treeInteraction?.select(event, page.id)} onDragEnter={page.page_type === "folder" && !invalidFolderTarget ? (event) => { event.preventDefault(); treeInteraction?.setDropTargetId(page.id, "inside"); } : undefined} onDragOver={page.page_type === "folder" && !invalidFolderTarget ? (event) => { event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = "move"; treeInteraction?.setDropTargetId(page.id, "inside"); } : undefined} onDragLeave={page.page_type === "folder" && !invalidFolderTarget ? (event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) treeInteraction?.setDropTargetId(null); } : undefined} onDrop={page.page_type === "folder" && !invalidFolderTarget ? (event) => treeInteraction?.drop(event, page.id) : undefined}>
