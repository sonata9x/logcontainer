...
export function WorkspaceSidebar({ workspaceId, workspaceName, nickname, pages, isSiteAdmin }: { workspaceId: string; workspaceName: string; nickname: string; accentColor: string; pages: WorkspacePage[]; isSiteAdmin: boolean }) {
  ...
  return <>
    ...
    <div className="sidebar-footer"><TrashPanel onChanged={reloadTree} /><button className="sidebar-action" onClick={() => setSettingsOpen(true)}><Settings size={15} />설정</button>{isSiteAdmin && <div className="sidebar-admin"><span><ShieldCheck size={15} />관리</span><Link className="sidebar-action" href="/workspace/admin/accounts">계정</Link><Link className="sidebar-action" href="/workspace/admin/bgm">BGM</Link></div>}<button className="sidebar-action" onClick={logout}><LogOut size={15} />로그아웃</button></div>
