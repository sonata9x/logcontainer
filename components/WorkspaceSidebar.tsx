...
function ShareDialog({ page, onClose }: { page: WorkspacePage; onClose: () => void }) {
  ...
  async function load() { const response = await fetch(`/api/resources/${page.id}/shares`); const result = await response.json(); setLoading(false); if (!response.ok) return setMessage(result.error ?? "공유 정보를 불러오지 못했습니다."); setShares(result.shares ?? []); setActorRole(result.actorRole); setAllowedRoles(result.allowedRoles ?? []); }
