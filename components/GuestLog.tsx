...
export function GuestLog({ token }: { token: string }) {
  ...
  async function exportTxt() { const response = await fetch(`/api/share/${encodeURIComponent(token)}/export`, { method: "POST" }); if (!response.ok) return window.alert("TXT를 만들지 못했습니다."); const blob = await response.blob(); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = `${payload?.page.title ?? "log"}.txt`; anchor.click(); URL.revokeObjectURL(url); }
