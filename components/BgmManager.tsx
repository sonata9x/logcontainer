...
export function BgmManager() {
  ...
  async function rename(item: BgmLibraryItem) {
    ...
    const response = await fetch("/api/bgm/library", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ id: item.id, title }) }); if (!response.ok) return window.alert("이름을 바꾸지 못했습니다."); await load();
  }
  ...
  async function createPlaylist() {
    const title = window.prompt("새 플레이리스트 이름")?.trim(); if (!title) return;
    const response = await fetch("/api/bgm/playlists", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ title }) }); if (!response.ok) return window.alert("플레이리스트를 만들지 못했습니다."); await load();
  }
  async function renamePlaylist(playlist: BgmPlaylist) {
    const title = window.prompt("플레이리스트 이름", playlist.title)?.trim(); if (!title) return;
    const response = await fetch("/api/bgm/playlists", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ id: playlist.id, title }) }); if (!response.ok) return window.alert("이름을 바꾸지 못했습니다."); await load();
  }
  ...
  async function addToPlaylist(playlist: BgmPlaylist) {
    if (!items.length) return window.alert("BGM 보관함이 비어 있습니다.");
    const labels = items.map((item, index) => `${index + 1}. ${item.custom_title || item.asset.canonical_title}`).join("\n");
    const number = Number(window.prompt(`추가할 BGM 번호\n${labels}`)); const item = items[number - 1]; if (!item) return;
    const response = await fetch("/api/bgm/playlists/items", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ playlistId: playlist.id, assetId: item.bgm_asset_id }) }); if (!response.ok) return window.alert("곡을 추가하지 못했습니다."); await load();
  }
  ...
  async function renamePlaylistItem(playlist: BgmPlaylist, itemId: string, currentTitle: string) {
    const customTitle = window.prompt("이 플레이리스트에서만 사용할 이름", currentTitle); if (customTitle == null) return;
    const response = await fetch("/api/bgm/playlists/items", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ playlistId: playlist.id, id: itemId, customTitle }) }); if (!response.ok) return window.alert("곡 별칭을 바꾸지 못했습니다."); await load();
  }
