...
import { bgmUsageLabel, formatBgmBytes, recentBgmUpload, type AdminBgmAsset } from "@/lib/bgm-admin";
...
export function AdminBgmPanel() {
  ...
  const load = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      const response = await fetch(`/api/admin/bgm?offset=${offset}&search=${encodeURIComponent(query)}`, { cache: "no-store", signal });
  ...
  return <section className="admin-accounts admin-bgm"><header><h1>BGM 관리</h1><p>사이트 전체의 MP3·YouTube BGM과 사용 위치를 확인합니다.</p></header>
    ...
    <div className="admin-bgm-list">{!loading && inventory?.assets.map((asset) => <article key={asset.id}><div className="admin-bgm-heading"><div><strong>{asset.title}</strong><small>{asset.sourceType === "upload" ? `MP3 · ${formatBgmBytes(asset.byteSize)}` : "YouTube · Storage 사용 없음"} · {asset.owner}</small></div><button className="button button-danger" aria-label={`${asset.title} 영구 삭제`} onClick={() => { setSelected(asset); setConfirmation(""); setError(""); }} disabled={pending || recentBgmUpload(asset)}><Trash2 size={15} /></button></div><span className="bgm-usage-badge">{bgmUsageLabel(asset)}</span>{recentBgmUpload(asset) && <small className="muted">업로드 후 24시간 동안 영구 삭제 보호</small>}
