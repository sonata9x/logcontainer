...
import { Archive, Download, EllipsisVertical, Eye, GripVertical, History, Info, MoreHorizontal, Music, Pencil, Plus, RotateCcw, Share2, Trash2, X } from "lucide-react";
...
export function LogEditor({ page, permissions, logId, entries, totalEntryCount, publication, importReport, initialAvatars = null }: { page: WorkspacePage; permissions: ResourcePermissions; logId: string; entries: LogEntry[]; totalEntryCount: number; publication: Publication | null; importReport: ImportSummary | null; initialAvatars?: SpeakerAvatarBundle | null }) {
  ...
  }, [liveEntries.length, loadMore, readMode, totalCount]);
  ...
  function prepareEntryDrag(entryId: string) {
    if (!permissions.canEdit || reorderPending) return false;
    dragOriginRef.current = liveEntries.map((entry) => ({ ...entry }));
    dragOrderRef.current = liveEntries;
    draggingEntryIdRef.current = entryId;
    setDraggingEntryId(entryId);
    return true;
  }

  function moveDraggedEntry(targetId: string) {
    const activeId = draggingEntryIdRef.current;
  ...
  async function commitEntryOrder() {
    ...
    if (!before || !draggingEntryIdRef.current) return;
  ...
  function endEntryDrag() {
    if (dragOriginRef.current) setLiveEntries(dragOriginRef.current);
    dragOriginRef.current = null;
    dragOrderRef.current = null;
    draggingEntryIdRef.current = null;
    setDraggingEntryId(null);
  ...
  function updateEntryPointerDrag(event: ReactPointerEvent<HTMLButtonElement>) {
    const pointer = pointerDragRef.current;
  ...
  function finishEntryPointerDrag(event: ReactPointerEvent<HTMLButtonElement>) {
    const pointer = pointerDragRef.current;
    if (!pointer || pointer.pointerId !== event.pointerId) return;
    pointerDragRef.current = null;
    pointerDragCleanupRef.current?.();
  ...
  return (
    <SpeakerAvatarContext.Provider value={{ avatars: speakerAvatars, setAvatars: setSpeakerAvatars }}><BgmPlayerProvider access={{ pageId: page.id }}>
      {readMode ? <SpeakerAvatarProvider avatars={speakerAvatars}><main className="public-log workspace-read-mode" data-font={pageExtras?.fontFamily ?? page.font_family ?? "pretendard"}>
        <header className="public-log-toolbar"><span>{title}</span><div className="toolbar-actions"><button className="button workspace-mode-toggle" aria-label="수정 모드로 전환" title="수정 모드로 전환" onClick={() => setReadMode(false)}><Pencil size={14} />수정 모드</button><HandoutLibrary mode="editor" pageId={page.id} canEdit={false} fontFamily={pageExtras?.fontFamily} /><PublicBgmMenu pageId={page.id} pageTitle={title} /></div></header>
        <h1>{title}</h1>
...
function PublicationDialog({ pageId, publication, onChange, onClose }: { pageId: string; publication: Publication | null; onChange: (publication: Publication | null) => void; onClose: () => void }) {
  ...
  async function save(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setPending(true); setError(""); const response = await fetch(`/api/pages/${pageId}/publication`, { method: current?.is_active ? "PATCH" : "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ visibility, currentPassword, newPassword }) }); const result = await response.json(); setPending(false); if (!response.ok) return setError(result.error ?? "게시 설정을 저장하지 못했습니다."); setCurrent(result); onChange(result); setCurrentPassword(""); setNewPassword(""); }
  ...
  const hasExistingPassword = Boolean(current?.id && current.visibility === "password");
  return <div className="modal-backdrop" onMouseDown={pending ? undefined : onClose}><section className="modal-card publication-modal" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" onClick={onClose} disabled={pending}><X size={17} /></button><h2>{current?.is_active ? "게시 설정" : "게시하기"}</h2>{publicUrl && <div className="publish-link-row"><a className="publish-url" href={publicUrl} target="_blank" rel="noreferrer">{publicUrl}</a><button className="button" type="button" onClick={() => navigator.clipboard.writeText(`${window.location.origin}${publicUrl}`)}>링크 복사</button></div>}<form onSubmit={save}><fieldset disabled={pending}><legend>공개 범위</legend><label className="checkbox-row"><input type="radio" name="visibility" checked={visibility === "public"} onChange={() => setVisibility("public")} /> 전체 공개</label><label className="checkbox-row"><input type="radio" name="visibility" checked={visibility === "password"} onChange={() => setVisibility("password")} /> 비밀글</label>{hasExistingPassword && <label className="field">현재 비밀번호<input type="password" minLength={4} maxLength={200} value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} required /></label>}{visibility === "password" && <label className="field">{hasExistingPassword ? "새 비밀번호 (변경하지 않으면 비워두기)" : "비밀번호"}<input type="password" minLength={hasExistingPassword ? undefined : 4} maxLength={200} value={newPassword} onChange={(event) => setNewPassword(event.target.value)} required={!hasExistingPassword} /></label>}</fieldset>{error && <p className="error">{error}</p>}<div className="modal-actions">{current?.is_active && <button className="button button-danger" type="button" onClick={stop} disabled={pending}>게시 중단</button>}<button className="button button-primary" disabled={pending}>{pending ? "게시 중…" : current?.is_active ? "게시 설정 저장" : "게시 시작"}</button></div></form></section></div>;
}
...
function TrashPanel({ pageId, onRestore, canEmpty }: { pageId: string; onRestore: (entry: LogEntry) => void; canEmpty: boolean }) {
  const [open, setOpen] = useState(false);
  return <><button className="button" onClick={() => setOpen(true)}><Trash2 size={14} />휴지통</button>{open && <TrashDialog endpoint={`/api/pages/${pageId}/trash`} kind="entries" canEmpty={canEmpty} onClose={() => setOpen(false)} onRestore={onRestore} />}</>;
}
