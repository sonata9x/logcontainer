...
-- 202608280001_security_hardening.sql
...
begin
  ...
  if window_seconds not between 1 and 86400 or max_requests not between 1 and 10000 or block_seconds not between 1 and 604800 then raise exception 'invalid rate limit policy'; end if;
  perform pg_advisory_xact_lock(hashtextextended(rate_scope || ':' || rate_key_hash, 0));
  select * into current_row from public.security_rate_limits where scope = rate_scope and key_hash = rate_key_hash for update;
  ...
  if current_row.window_started_at + make_interval(secs => window_seconds) <= rate_now then
    update public.security_rate_limits set window_started_at = rate_now, request_count = 1, blocked_until = null, updated_at = rate_now
...
begin
  if coalesce(auth.role(), '') <> 'service_role' then raise exception 'permission denied'; end if;
  delete from public.security_rate_limits where updated_at < now() - interval '2 days' and (blocked_until is null or blocked_until < now());
  get diagnostics purged_count = row_count;
  return purged_count;
end;
$$;
...
-- 202608280002_fix_security_rate_limit_timestamp.sql
