...
import { applyCorrections, parseCorrectionSettings } from "@/lib/logs/corrections";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  ...
  const settings = parseCorrectionSettings(await request.json().catch(() => null));
  if (!settings) return NextResponse.json({ error: "TXT 교정 옵션이 올바르지 않습니다." }, { status: 400 });
  ...
  const text = applyCorrections((data ?? []).map((entry) => toLogEntryDto(entry as Record<string, unknown>)), settings);
