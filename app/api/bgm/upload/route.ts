...
import { BGM_AUDIO_BUCKET, normalizeBgmOriginalFilename, validBgmUpload } from "@/lib/bgm";
...
export async function POST(request: Request) {
  ...
  const originalFilename = normalizeBgmOriginalFilename(body.originalFilename);
  if (!title || !originalFilename || !validBgmUpload(body.mimeType, body.byteSize)) return NextResponse.json({ error: "25MB 이하 MP3 파일과 제목을 확인해주세요." }, { status: 400 });
  const path = `${context.user.id}/${randomUUID()}.mp3`;
  const { data: asset, error } = await context.supabase.from("bgm_assets").insert({ owner_user_id: context.user.id, source_type: "upload", storage_path: path, canonical_title: title, original_filename: originalFilename, mime_type: "audio/mpeg", byte_size: body.byteSize, is_ready: false }).select("id").single();
