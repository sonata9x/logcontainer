...
export async function GET(request: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  ...
  try {
    const [extras, bgmItems] = await Promise.all([serializePageExtras(context.admin, page), getPageBgmItems(context.admin, context.page.id)]);
    return NextResponse.json({ pageId: context.page.id, extras, bgmItems }, { headers: { "Cache-Control": "private, no-store" } });
  } catch {
