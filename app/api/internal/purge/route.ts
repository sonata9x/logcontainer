...
export async function GET(request: Request) {
  ...
  try {
    const [{ data, error }, { data: events, error: eventError }, { data: rateLimits, error: rateLimitError }, purgedImportUploads] = await Promise.all([
    ...
    return NextResponse.json({ purged: data ?? 0, purgedLogEvents: events ?? 0, purgedRateLimits: rateLimits ?? 0, purgedImportUploads });
  } catch (error) {
