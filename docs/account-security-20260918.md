...
추가 UI 조정: 사용자 inline style이 포함된 block-flow Rich 콘텐츠 메시지에만 바깥 위·아래 여백을 각각 6px 추가했다. 일반 대화/inline 배지 간격, 사용자 CSS padding/top/bottom, 중앙 정렬, canonical document는 변경하지 않는다. 편집·게시 공통 Roll20 테마에 적용한다.
